...
export class PermissionService {
  static async checkDescriptionPermissions(
    showId: string,
    dogId: string,
    userRole: UserRole,
  ...
  static async checkShowStatus(showId: string): Promise<ShowStatus> {
    try {
  ...
  static isShowEditable(status: ShowStatus): boolean {
    return status === "in_progress" || status === "open_for_registration";
  }

  static isShowCompleted(status: ShowStatus): boolean {
