...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const parsedParams = {
      ...
      gdpr_consent: queryParams.gdpr_consent
        ? queryParams.gdpr_consent === "true"
        : undefined,
    };

    // Validate query parameters
    const validatedParams = ownerQuerySchema.parse(parsedParams);

    // Get owners using service
    const ownerService = new OwnerService(supabaseClient);
    const result = await ownerService.getMany(validatedParams);
